...
<table class="table table-striped list">
	<thead class="thead-dark">
		<tr>
			...
			<th class="title">{{ __('Tên danh mục') }}</th>
			<th>{{ __('Mô tả') }}</th>
		...
		@if ($cates->isNotEmpty())
			@foreach($cates as $key => $item)
				<tr id="item-{{ $item->id }}">
					...
					<td class="title"><a href="{{ route('editCategoryAdmin', $item->id) }}">{{ $item->title }}</a></td>
					<td>{{ $item->content }}</td>
					<td class="action">
						<a href="{{ route('editCategoryAdmin', $item->id) }}" class="edit"><i class="fal fa-edit"></i></a>
						<a href="{{ route('deleteCategoryAdmin', $item->id) }}" class="delete btn-delete"><i class="fal fa-times"></i></a>
