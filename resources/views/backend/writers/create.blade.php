...
@section('content')
	<div id="create-writer" class="container catProduct-page product-page route">
		<div class="head">
			...
			<h1 class="title">{{ _('Thêm tác giả') }}</h1>
		</div>
		<div class="main">
			<form action="{{ route('storeWriterAdmin') }}" class="dev-form activity-form" method="POST" name="create_writer" role="form">
				{{ csrf_field() }}
				...
				<div class="group-action">
					...
					<a href="{{ route('writersAdmin') }}" class="btn btn-cancel">{{ _('Hủy') }}</a>
				</div>
			</form>
		</div>
	</div>
