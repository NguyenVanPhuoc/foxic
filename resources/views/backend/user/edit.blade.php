			...
			<div class="row">
				<div class="col-md-9 content clearfix">
					...
					<div class="form-group" id="frm-phone">
						<label for="phone">{{ _('Số điện thoại') }}<small>(*)</small></label>
						...
						<div class="col-md-6" id="frm-roles">
							...
							<select name="role">
								...
								@foreach($roles as $role)
									<option value="{{$role->name}}" {{ isset($name_role[0]) && ($role->name==$name_role[0]) ? 'selected' : ''}}>{{$role->name}}</option>
								@endforeach
								@endif
							</select>
						</div>
				...
				<div class="col-md-3 sidebar clearfix">
						...
						<div id="frm-levels" class="desc img-upload">
							...
							<option value="">{{ __('Chọn cấp độ') }}</option>
								@foreach($levels as $item)
									<option value="{{ $item->id }}" {{ $user->level_id == $item->id ? 'selected' : ''}}>{{$item->title}}</option>
								@endforeach
							</select>
						</div>
					</section>
					<section id="frm-type" class="box-wrap">
						...
						<select name="type_author">
							<option value="">--Chọn--</option>
							@foreach($type_author as $key => $type)
								<option value="{{$key}}" {{ $user->type_author ==  $key ?  'selected' : ''}}>{{$type}}</option>
							@endforeach
						</select>
					</section>
				</div>
