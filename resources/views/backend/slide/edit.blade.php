...
@section('content')
<div id="edit-slide" class="content-wrapper media-category slides">
   <!-- Main content -->
   <section class="content">
      <div class="container">
         <div class="head">
            <a href="{{route('slidesAdmin')}}" class="back-icon"><i class="fas fa-angle-left" aria-hidden="true"></i>{{__('Tất cả')}}</a>
            <h1 class="title">{{__('Sửa slide')}}</h1>
         </div>
         <div class="main">
            @include('notices.index')
            <form action="{{route('editSlideAdmin',['id'=>$slide->id])}}" method="POST" name="editSlide" data-toggle="validator" role="form" class="dev-form">
                {{ csrf_field() }}
               <div id="frm-title" class="form-group">
                  <label for="title">{{ __('Name') }}</label>
                  <input type="text" name="title" class="form-control" placeholder="Enter slide name" value="{{$slide->title}}" required>
               ...
               <div id="list-item" class="card card-outline card-info">
                  ...
                  <div class="card-body sortable-items list-slide">
                     <div class="list">
                      ...
                      @endphp
                        @if($list_content) @php $count=1; @endphp
                        <ul class="sortable" data-recores="{{count($list_content)}}">
                            @foreach($list_content as $content)
                           <li class="row item" data-position="{{$count}}">
                              <i class="fas fa-trash" aria-hidden="true"></i>
                              <div id="img-{{$count}}" class="col-md-3 img-upload">
                                 <div class="image full-width">
                                    <a href="{{ route('loadMedia') }}" class="library"><i class="fa fa-edit" aria-hidden="true"></i></a>
                                    {!!image($content->image,325,275,'')!!}
                                    <input type="hidden" name="thumbnail" class="thumb-media" value="{{$content->image}}" />
                                 </div>
                              </div>
                             {{--  <div class="col-md-9 desc"><textarea name="desc" class="form-control" rows="10">{!!$content['text']!!}</textarea></div> --}}
                              <div class="col-md-12 link"><input type="text" name="link" class="form-control" value="{{$content->link}}" /></div>
                           </li>
                           @php $count++; @endphp
                           @endforeach
                        </ul>
                     ...
                     <button type="button" class="btn btn-primary add-row"><i class="fa fa-plus-circle" aria-hidden="true"></i>{{__('Add row')}}</button>
                  </div>
               </div>
...
<script type="text/javascript">
   $(document).ready(function(){
       $("#edit-slide").on('click','.dev-form .group-action .btn-success',function(){
