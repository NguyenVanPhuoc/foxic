...
@section('title','Gamebank config')
@section('content')
@php $coins = getCoins('gamebank'); @endphp
<div id="gamebank-api" class="page p-api">
	<div class="head container">
		<h1 class="title">Gamebank API</h1>		
	</div>	
	<div class="main">
		<ul class="nav-filter">
			<li class="active"><a href="{{route('APIAdmin',['type'=>'gamebank'])}}">Gamebank</a></li>
			<li><a href="{{route('APIAdmin',['type'=>'atm'])}}">ATM</a></li>
		</ul>
		<form action="{{route('editAPIAdmin',['type'=>'gamebank'])}}" method="POST" name="blog" class="dev-form">
			{!!csrf_field()!!}
			<div class="row api-info">
				<div class="col-md-4 left-box clearfix">
					<div class="wrap">
						<h3 class="title-h3">Thông số kết nối</h3>
						<div id="frm-merchant" class="form-group">
							<label for="">Merchant ID<small class="required">*</small></label>
							<input type="text" name="merchant" class="form-control" value="{{$options->gamebank_merchant}}">
						</div>
						<div id="frm-user" class="form-group">
							<label for="">User api<small class="required">*</small></label>
							<input type="text" name="userApi" class="form-control" value="{{$options->gamebank_userApi}}">
						</div>
						<div id="frm-password" class="form-group">
							<label for="">Password api<small class="required">*</small></label>
							<input type="text" name="passApi" class="form-control" value="{{$options->gamebank_passwordApi}}">
						</div>
					</div>
				</div>
	...
	$(function() {
		...
		$("#frm-meta").on('click','.btn-add',function(){		
			...
			var html = '<li class="ui-state-default item-'+number+' new" data-position="'+number+'">';
					...
					html = html + '<div class="col-md-8 col-sm-8 col-xs-8 frm-money"><input type="text" name="money_'+number+'" placeholder="Số tiền cần quy đổi" class="frm-input txt-number"/></div>';
					html = html + '<div class="col-md-4 col-sm-4 col-xs-4 clearfix frm-coin "><input type="text" name="coin_'+number+'" placeholder="Số coin" class="frm-input txt-number" /><i class="fa fa-trash" aria-hidden="true"></i></div>';
					html = html + '</div>';
				html = html + '</li>';
		...
		$("#gamebank-api").on('click','form .group-action button',function(){
			...
			var merchant = $("#frm-merchant input").val();
			var userApi = $("#frm-user input").val();
			var passwordApi = $("#frm-password input").val();
			...
			// console.log(111);
			if(merchant=="") errors.push('Vui lòng nhập Merchant ID.');
			if(userApi=="") errors.push('Vui lòng nhập User API.');
			if(passwordApi=="") errors.push('Vui lòng mật khẩu API.');
			var i;
	   		var html = "<ul>";
